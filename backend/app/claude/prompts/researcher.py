...
3. 주제의 **핵심 행위/목적**(예: "게임 만들기", "서버 구축", "모델 학습")을 다루는 자료를 우선하세요.
4. relevance_score 0.7 미만은 포함하지 마세요.
5. 검색 시 제공된 쿼리 힌트를 참고하되, 주제에 더 적합한 검색어를 자체적으로 만들어 추가 검색하세요."""
...
[GOAL]
주제에 대한 **공식 가이드, 튜토리얼, 학술 논문, 표준 문서** 중 주제를 직접 다루는 자료를 웹 검색하여 JSON으로 반환하세요.
...
**제외 대상**: 단순 API 레퍼런스 페이지(예: pygame.Rect, numpy.array 등), 도구의 일반 홈페이지, 설치 가이드
...
[GOAL]
주제와 관련된 **실제 구현 프로젝트, 예제 코드, 튜토리얼 리포지토리**를 웹 검색하여 JSON으로 반환하세요.
...
**제외 대상**: 주제에서 사용하는 도구/프레임워크 자체의 공식 리포지토리 (예: pygame/pygame, python/cpython)
...
[GOAL]
주제에 대한 **실전 튜토리얼, 심화 해설, 프로젝트 가이드** 영문 블로그 글을 웹 검색하여 JSON으로 반환하세요.
...
[GOAL]
주제에 대한 **한국어 튜토리얼, 프로젝트 후기, 기술 해설** 블로그 글을 웹 검색하여 JSON으로 반환하세요.
