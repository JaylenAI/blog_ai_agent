...
async def test_publisher_success_no_vault() -> None:
    stage, _ = _make_stage()
    result = await stage.execute(_make_input())

    assert result.success is True
    assert result.data["obsidian_saved"] is False
    assert result.data["tistory_ready"] is False
...
async def test_publisher_saves_to_obsidian() -> None:
...
async def test_publisher_obsidian_nonexistent_vault() -> None:
